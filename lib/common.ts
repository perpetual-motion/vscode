...
import { setVerbose, verboseEnabled } from './constants';

export let $cmd = 'main';

export const $allArgs = process.argv.slice(2);
...
setVerbose($switches.includes('--verbose'));

export function getSwitch(sw: string): string | boolean | undefined {
  if (!sw.startsWith('--')) {
    sw = '--' + sw;
  }
  const swe = `${sw}=`;
  ...
  for (let i = 0; i < $switches.length; i++) {
    ...
    if ($switches[i].startsWith(swe)) {
      let v = $switches[i].substring(swe.length, $switches[i].length);
      $switches.splice(i, 1);
      return v;
    }
  }
...
process.on('unhandledRejection', (reason: any, _promise) => {
  error(`${reason?.stack?.split(/\r?\n/).filter((l: string | string[]) => !l.includes('node:internal') && !l.includes('node_modules')).join('\n')}`);
  process.exit(1);
});
...
export async function updateFiles(files: string[], dest: string | Promise<string>) {
  ...
  await Promise.all(files.map(async (each) => {
    const sourceFile = await filepath.isFile(each);
    if (sourceFile) {
...
export async function assertAnyFolder(oneOrMoreFolders: string | string[], errorMessage?: string): Promise<string> {
  ...
  for (const each of oneOrMoreFolders) {
    const result = await filepath.isFolder(each);
    if (result) {
...
export async function assertAnyFile(oneOrMoreFiles: string | string[], errorMessage?: string): Promise<string> {
  ...
  for (const each of oneOrMoreFiles) {
    const result = await filepath.isFile(each);
    if (result) {
      verbose(`Folder ${brightGreen(each)} exists.`);
      return result;
    }
